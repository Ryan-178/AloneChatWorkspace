...
def init_command(obj: dict, force: bool, api_key: str | None) -> None:
    """
    初始化AloneWork配置 / Initialize AloneWork config

    创建.aloneworkrc配置文件，配置API密钥 / Create .aloneworkrc config file
    """
    console.print(Panel.fit(
        "[bold cyan]AloneWork 配置初始化 / Config Initialization[/bold cyan]\n\n"
        "本工具将帮助你 / This tool will help you:\n"
        "1. 创建配置文件 (.aloneworkrc) / Create config file\n"
    ...
    key_file = config_path.parent / ".alonework_key"
    ...
    console.print("  $ alonework chat        # 启动交互式对话 / Start interactive chat")
    console.print("  $ alonework generate    # 生成代码 / Generate code")
    console.print("  $ alonework commit      # 智能提交 / Smart commit")
    console.print("  $ alonework --help      # 查看所有命令 / View all commands")
