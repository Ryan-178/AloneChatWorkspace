"""
æéç®¡çå?/ Permission Manager

ç®¡çå·¥å·æéåè§å?/ Manages tool permissions and rules
"""
...
from alonework.permissions.rules import (
    PermissionRule,
    PermissionMode,
    PermissionAction,
...
class PermissionManager:
    ...
    def set_mode(self, mode: PermissionMode) -> None:
        """è®¾ç½®æéæ¨¡å¼ / Set permission mode"""
        self.mode = mode
        self._save_config()
    ...
    def is_allowed(self, tool_name: str, command: Optional[str] = None) -> bool:
        """
        æ£æ¥å·¥å·æ¯å¦è¢«åè®¸ / Check if tool is allowed
    ...
    def needs_prompt(self, tool_name: str) -> bool:
        """æ£æ¥æ¯å¦éè¦æç¤ºç¨æ?/ Check if needs user prompt"""
        if self.mode == PermissionMode.ACCEPT:
            return False
    ...
    def _match_tool(
        ...
        """å¹éå·¥å·æ¨¡å¼ / Match tool pattern"""
    ...
    def get_allowed_tools(self) -> list[str]:
        """è·ååè®¸çå·¥å·åè¡?/ Get allowed tools list"""
        return list(self._allowed)
    
    def get_denied_tools(self) -> list[str]:
        """è·åæç»çå·¥å·åè¡?/ Get denied tools list"""
        return list(self._denied)
    
    def get_tool_description(self, tool_name: str) -> str:
        """è·åå·¥å·æè¿° / Get tool description"""
        return TOOL_DESCRIPTIONS.get(tool_name, tool_name)
    
    def show_status(self) -> None:
        """æ¾ç¤ºæéç¶æ?/ Show permission status"""
        from rich.table import Table
        
        console.print("\n[bold cyan]æéç¶æ?/ Permission Status[/bold cyan]\n")
        
        table = Table(show_header=True)
        table.add_column("å·¥å· / Tool", style="cyan")
        ...
        for tool in sorted(all_tools):
            ...
            if tool in self._allowed:
                status = "[green]åè®¸ / Allowed[/green]"
            elif tool in self._denied:
                status = "[red]æç» / Denied[/red]"
            else:
                status = "[yellow]æç¤º / Prompt[/yellow]"
            table.add_row(tool, desc, status)
        
        console.print(table)
        console.print(f"\n[dim]æ¨¡å¼ / Mode: {self.mode.value}[/dim]")
        console.print()
