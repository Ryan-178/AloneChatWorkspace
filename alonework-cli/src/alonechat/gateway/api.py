...
from alonechat.core.types import AgentMode, TaskStatus
from alonechat.core.task import Task, Artifact
from alonechat.agent.mtc_agent import MTCAgent
from alonechat.agent.code_agent import CodeAgent
from alonechat.tools.skills_registry import SkillsRegistry, register_builtin_skills
from alonechat.tools.skills_marketplace import SkillsMarketplace
from alonechat.gateway.session import SessionManager
from alonechat.gateway.user_api import create_user_api, get_current_user_optional
from alonechat.gateway.mcp_api import create_mcp_api
...
class TaskManager:
    ...
    def create_task(self, description: str, priority: str = "medium") -> Task:
        """创建任务 - Create task"""
        from alonechat.core.types import TaskPriority
...
@app.get("/api/mode")
async def get_mode(session_id: Optional[str] = Query(None), user: Optional[dict] = Depends(get_current_user_optional)):
    """获取当前模式 - Get current mode"""
...
async def switch_mode(request: ModeSwitchRequest, user: Optional[dict] = Depends(get_current_user_optional)):
    """切换模式 - Switch mode"""
...
async def create_task(request: TaskCreateRequest, user: Optional[dict] = Depends(get_current_user_optional)):
...
async def get_task(task_id: str, user: Optional[dict] = Depends(get_current_user_optional)):
...
async def cancel_task(task_id: str, user: Optional[dict] = Depends(get_current_user_optional)):
    """取消任务 - Cancel task"""
...
async def get_skill(skill_name: str, user: Optional[dict] = Depends(get_current_user_optional)):
...
async def execute_skill(request: SkillExecuteRequest, user: Optional[dict] = Depends(get_current_user_optional)):
...
async def list_artifacts(user: Optional[dict] = Depends(get_current_user_optional)):
    """列出产出物 - List artifacts"""
...
async def get_artifact(artifact_id: str, user: Optional[dict] = Depends(get_current_user_optional)):
...
@app.get("/api/stats")
async def get_stats(user: Optional[dict] = Depends(get_current_user_optional)):
...
async def search_remote_skills(query: str = Query(..., description="搜索关键词 / Search keyword"), user: Optional[dict] = Depends(get_current_user_optional)):
...
async def get_trending_skills(user: Optional[dict] = Depends(get_current_user_optional)):
...
async def list_installed_remote_skills(user: Optional[dict] = Depends(get_current_user_optional)):
...
async def install_remote_skill(request: RemoteSkillInstallRequest, user: Optional[dict] = Depends(get_current_user_optional)):
...
async def uninstall_remote_skill(skill_name: str, user: Optional[dict] = Depends(get_current_user_optional)):
