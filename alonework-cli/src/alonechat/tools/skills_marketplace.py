...
from alonechat.tools.skills_registry import SkillsRegistry, SkillMetadata
from alonechat.tools.skills_sh_client import (
    SkillsShClient,
    GitHubUrlParser,
    RemoteSkill,
)
...
class SkillsMarketplace:
    ...
    async def install_from_remote(
        ...
        if global_install:
            install_path = self._skills_dir / _sanitize_skill_name(actual_skill_name)
        else:
            install_path = Path.cwd() / ".skills" / _sanitize_skill_name(actual_skill_name)
    ...
    def uninstall_remote(self, name: str) -> Dict[str, Any]:
        ...
        local_path = Path.cwd() / ".skills" / safe_name
        global_path = self._skills_dir / safe_name
