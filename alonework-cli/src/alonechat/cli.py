...
@click.version_option(version=__version__, prog_name="AloneWork")
...
def main(
    ...
    """
    AloneWork - 国产化、终端原生、深度中文优化的AI编程Agent
    ...
    \b
    快速开始 / Quick Start:
    $ alonework init          # 初始化配置 / Initialize config
    $ alonework chat          # 启动对话 / Start chat
    $ alonework generate      # 生成代码 / Generate code
    $ alonework -p "query"    # 打印模式 / Print mode
    $ alonework -C            # 继续会话 / Continue session
    $ alonework --name "我的会话"  # 设置会话名称 / Set session name
    $ alonework --agent model=deepseek-v3  # 覆盖代理设置 / Override agent
    $ alonework --no-stream   # 禁用流式输出 / Disable streaming (v2.1.78)

    \b
    数据管理 / Data Management:
    $ alonework data collect           # 收集交互数据 / Collect interaction data
    $ alonework data list              # 列出已收集数据 / List collected data
    $ alonework data export            # 导出训练数据 / Export training data
    $ alonework data quality           # 评估数据质量 / Evaluate data quality
    $ alonework data stats             # 数据统计 / Data statistics
    $ alonework data clean             # 清理数据 / Clean up data

    \b
    工作流编排 / Workflow Orchestration:
    $ alonework workflow list          # 列出工作流 / List workflows
    $ alonework workflow create        # 创建工作流 / Create workflow
    $ alonework workflow run <id>      # 执行工作流 / Execute workflow
    $ alonework workflow plan "task"   # 规划任务 / Plan task
    $ alonework workflow delete <id>   # 删除工作流 / Delete workflow

    \b
    环境管理 / Environment Management:
    $ alonework env status             # 查看环境状态 / View environment status
    $ alonework env reset              # 重置环境 / Reset environment
    $ alonework env checkpoint         # 创建检查点 / Create checkpoint
    $ alonework env restore <name>     # 恢复检查点 / Restore checkpoint
    $ alonework env sandbox            # 沙箱管理 / Sandbox management
    $ alonework env tree               # 显示状态树 / Display state tree

    \b
    CLI 增强 / CLI Enhancements:
    $ alonework --worktree /path/to/worktree  # 在隔离工作树中启动 / Start in isolated worktree (v2.1.49)
    $ alonework --add-dir /path/to/dir1 --add-dir /path/to/dir2  # 附加目录 / Additional dirs (v2.1.45)
    $ alonework --show-thinking  # 启用实时思维块显示 / Enable live thinking display (v2.1.0)
    """
    ...
    if verbose:
        console.print(f"[dim]AloneWork v{__version__}[/dim]")
        if config:
...
from alonechat.commands.codex import codex as codex_commands
main.add_command(codex_commands, name="codex")
